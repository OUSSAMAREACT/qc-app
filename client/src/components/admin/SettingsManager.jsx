    ...
    const handleSave = async () => {
                ...
                <Card className="p-6 bg-white dark:bg-gray-800">
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Debug Section */}
                        <div className="mt-8 p-4 bg-gray-100 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
                            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Zone de Debug</h3>
                            <p className="text-xs text-gray-500 mb-2">Votre Rôle Actuel : <span className="font-mono font-bold text-blue-600">{token ? JSON.parse(atob(token.split('.')[1])).role : 'Inconnu'}</span></p>
                            <Button
                                onClick={async () => {
                                    try {
                                        let apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001/api';
                                        ...
                                        const res = await fetch(`${apiUrl}/debug/promote`, {
                                            method: 'POST',
                                            headers: { 'Authorization': `Bearer ${token}` }
                                        });
                                        if (res.ok) {
                                            alert("Vous êtes maintenant ADMIN ! Veuillez vous reconnecter.");
