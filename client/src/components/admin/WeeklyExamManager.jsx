...
import { Plus, Calendar, CheckSquare, Trash2, Users, Trophy } from 'lucide-react';
...
export default function WeeklyExamManager() {
    ...
    const handleCategoryChange = async (categoryId) => {
        setFormData(prev => ({ ...prev, selectedCategory: categoryId, selectedQuestions: [] }));
        if (!categoryId) {
            setQuestions([]);
            return;
        }
        ...
        <div className="space-y-8">
            ...
            {isCreating ? (
                <Card className="p-6 border-blue-100 dark:border-blue-900/30 bg-white dark:bg-gray-800 animate-in fade-in slide-in-from-top-4">
                    <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Créer un nouvel examen</h3>
                        ...
                        <div className="border-t border-gray-100 dark:border-gray-700 pt-6">
                            ...
                            {questions.length > 0 && (
                                <div className="max-h-60 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-2 space-y-2">
                                    {questions.map(q => (
                                        <div key={q.id} className="flex items-start gap-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded">
                                            <input
                                                type="checkbox"
                            ...
                            <p className="text-sm text-gray-500 mt-2">
                                {formData.selectedQuestions.length} questions sélectionnées
                            </p>
                        </div>

                        <div className="flex justify-end gap-3">
                            <Button type="button" variant="ghost" onClick={() => setIsCreating(false)}>Annuler</Button>
                            <Button type="submit" disabled={formData.selectedQuestions.length === 0}>Créer l'examen</Button>
                        </div>
                    </form>
                </Card>
            ) : (
